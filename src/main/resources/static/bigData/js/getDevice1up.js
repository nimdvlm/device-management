...
dataMap.dataGDP = {
    ...
    2002:[4315,2150.76,6018.28,2324.8,1940.94,5458.22,2348.54,3637.2,5741.03,10606.85,8003.67,3519.72,4467.55,2450.48,10275.5,6035.48,4212.82,4151.54,13502.42,2523.73,642.73,2232.86,4725.01,1243.43,2312.82,162.04,2253.39,1232.03,340.65,377.16]
};
...
dataMap.dataPI = {
    ...
    2002:[82.44,84.21,956.84,197.8,374.69,590.2,446.17,474.2,79.68,1110.44,685.2,783.66,664.78,535.98,1390,1288.36,707,847.25,1015.08,601.99,222.89,317.87,1047.95,281.1,463.44,39.75,282.21,215.51,47.31,52.95]
};
...
dataMap.dataSI = {
    ...
    2002:[1249.99,1069.08,2911.69,1134.31,754.78,2609.85,943.49,1843.6,2622.45,5604.49,4090.48,1337.04,2036.97,941.77,5184.98,2768.75,1709.89,1523.5,6143.4,846.89,148.88,958.87,1733.38,481.96,934.88,32.72,1007.56,501.69,144.51,153.06]
};
...
dataMap.dataTI = {
    ...
    2002:[2982.57,997.47,2149.75,992.69,811.47,2258.17,958.88,1319.4,3038.9,3891.92,3227.99,1399.02,1765.8,972.73,3700.52,1978.37,1795.93,1780.79,6343.94,1074.85,270.96,956.12,1943.68,480.37,914.5,89.56,963.62,514.83,148.83,171.14]
};
...
dataMap.dataEstate = {
    ...
    2002:[298.02,73.04,140.89,65.83,51.48,130.94,76.11,118.7,384.86,371.09,360.63,139.18,188.09,125.27,371.13,199.31,145.17,165.29,808.16,82.83,21.45,90.48,210.82,53.49,95.68,3.42,77.68,41.52,9.74,13.46]
};
...
dataMap.dataFinancial = {
    ...
    2002:[561.91,76.86,179.6,124.1,48.39,137.18,75.45,31.6,485.25,368.86,347.53,81.85,138.28,76.51,310.07,158.77,96.95,92.43,454.65,35.86,10.08,134.52,183.13,41.45,102.39,2.81,67.3,42.08,16.75,21.45]
};
...
option0 = {
    ...
    baseOption: {
        timeline: {
            ...
            // },
            data: [
                '压力传感器','温度传感器','湿度传感器', '形变传感器', '速率传感器','光照传感器'
            ],
            label: {
        ...
        legend: {
            x: 'right',
            data: ['最大值', '均值', '最小值', '标准差', '设备数量', '正常设备数量'],
            selected: {
                '标准差': false, '设备数量': false, '正常设备数量': false
            }
        },
        calculable : true,
        grid: {
...
function getdata() {
    ...
    if (myXmlHttpRequest) {
        ...
        var url = "/api/device/alldevices?limit=14";// /api/device/alldevices?limit=20http://10.108.219.218:8100/api/v1/tenant/devices/2?limit=20;http://10.108.219.218:80/api/device/alldevices?limit=20
        ...
        if (myXmlHttpRequest.readyState == 4) {
            ...
            for(var i=0; i<meso.length; i++){
                ...
                if(Math.floor((i+1)/2)%2==1){
                    if(i%2==1){//c8856b
                        div.setAttribute("style","background:#fffae3;width: 30.55%;height:200px;float: left;margin:20px 2.77% 20px 2.77%");//background-image:url('images/bgimage1.png')
                    }else {
                        div.setAttribute("style","background:#fffae3;width: 36.11%;height:200px;float: left;margin:20px 2.77% 20px 2.77%");//background-image:url('images/bgimage1.png')
                    }
            ...
            div1.setAttribute("style","display:none;width:92.59%;height:400px;margin-top:20px");
            $(div1).fadeIn(3000);
