...
+function($) {
    $(function() {
        ...
        theme = theme ? theme : 'bg-black';
        $('#plat_header').addClass(theme);
        $('#nav').addClass(theme);
        ...
        }).on('click', '.btn-bg', function (evt) {
            var color = $(evt.target).data('theme');
            var $header = $('#plat_header');
            var $nav = $('#nav');
            var theme = window.localStorage.getItem('theme');
            theme = theme ? theme : 'bg-black';
            if (theme.indexOf(color) < 0) {
                $header.removeClass(theme);
                $nav.removeClass(theme);
                $header.addClass('bg-' + color);
                $nav.addClass('bg-' + color);
                window.localStorage.setItem('theme', 'bg-' + color);
            }
        });
